...
| 言葉が減った・消えたように見える | 設定 →「言葉の保存方式」 | 「年ファイル（N 年分）」の N が 0 なら索引が取れていない → 「今すぐ同期」。直らなければ 2.4。 |
...
### 2.4 保存方式を戻す（年ファイル・月ファイルをやめる）

「言葉」と「記録」は、それぞれ **旧方式（data.json に全部入り）** と **新方式（年ごと・月ごとのファイル）** を設定画面のボタンで行き来できる。どちらも片道切符ではない。

- 設定 → 1. 同期 →「**言葉を data.json 方式に戻す**」: 全年のファイルを読み込み、`data.json` に書き戻す。年ファイルは残るが使われない。
...
| コミット | 内容 |
|---|---|
| （最新） | v7.3 タブバーが浮く不具合の修正（ページ固定 + `#shell` スクロール） |
...
**注意**: 年ファイル方式のまま `521a963` より前へ、月ファイル方式のまま `1773083` より前へ戻すと、そのアプリはファイルを読めず言葉・記録が見えなくなる。**先に 2.4 で旧方式に戻してから**アプリを戻すこと。
...
|---|---|---|---|
| 言葉（ノート） | `notes:YYYY`（作成年ごと。全年を端末に持つ）、未同期の年 `notesDirty`、最後に見た索引 `notesStamps` | `kotoba-data` の `notes/YYYY.json` と `notes/index.json` | 年ファイル方式のとき。旧方式では `data.json` の `notes` |
...
```
{ version: 1, year: 'YYYY', updatedAt, notes: [...], deletedIds: [{id, deletedAt}] }
```
**索引 `notes/index.json`**: `{ version: 1, updatedAt, years: { 'YYYY': { updatedAt, count } } }`
...
- **言葉（年ファイル）**: `notes/index.json` を読む → 対象の年（端末で変えた年・他端末が変えた年・端末に無い年）を 1 つずつ読む → `mergeNoteSets`（id ごとに `updatedAt` の新しい方。削除は墓標の時刻と比べる）→ 変わっていれば書く → 索引を書く。端末側は保存のたびに年ごとの JSON を前回と比べ、変わった年だけ IndexedDB に書いて「未同期」にする。
...
| 壁 | 目安 | 何が起きるか | 対策 |
|---|---|---|---|
| 1 年分の言葉が約 1,000 枚（年ファイル 1 個が 3 MB）を超える | 1 日 3 枚以上のペースが続いたとき | その年のファイルの読み書きが遅くなる | `notesYearPath` と `yearOfNote` を半年や四半期の粒度に変える |
