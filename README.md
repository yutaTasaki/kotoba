...
{ id: 'w_YYYYMMDD_連番'（日付の部分は eventDate。ここから月ファイルが引ける）,
  ...
  flow: { from, to, what, amount（出典の文字列のまま）, value, unit,
           ...
           strata: [地層id]（v15。この金が動かした地層。0〜2件） } | null（v12。換算しない）,
...
{ version:1, updatedAt,
  strata: [{ id:'st_jp_rate_long'（手で付ける英字 id。生成はこれを返す）,
             ...
             levels: [{value, at, note, source, url, via:'card'|'probe'|'hand', eventId}]（新しい順・全部残す）,
             active, order, createdAt, updatedAt }] }
```
数十年単位でしか動かない量を13件だけ持つ。**生成には作らせない**（増えると意味がなくなる）ので、プロンプトには id 付きの一覧を差し込み、`strata` に id を返させる。閉じたリストなので表記ゆれが起きない。取り込み時に**一覧に無い id は落とす**。`levels` は上書きせず全部残す（地層の値そのものが「溜まるもの」なので、上書きすると、この機能が防ごうとしている当のものを自分でやることになる）。
**値は札が運んでくる（v15）**：生成は札ごとに `levels`（`{stratumId, value, at}`）を返し、取り込みが地層に足す。13件のうち12件が「値は未記入」で、四半期に一度手で入れる設計が止まっていた——札を取り込むたびに埋まるようにした。縛りは4つ：(1) **出典に数字が書かれているときだけ**（推測・概算は書かせない）、(2) `source`・`url` は**札のものを使う**（生成に書かせると、そこだけ未検証の URL が入る。札の url は取り込み時点で検証済み）、(3) **同じ日付が既にあれば落とす**（履歴なので足すだけ。`via:'card'` のときだけ。手で入れるぶんは打ち間違いを直せないと困るので上書きしてよい）、(4) 一覧に無い `stratumId` は落とす。取り込み結果に「地層の値 N件を入れました／同じ日付で重なった M件は落としました」を出す。札から入った値は `value` が出典の書き方のまま（「1兆2075億ドル」）なので**地層の `unit` を後ろに足さない**（「1兆2075億ドル 億ドル」になる）。
**専用プロンプトで残りを埋める（v16 B）**：札から埋まるのは13件のうちせいぜい5件で、週のニュースに出てこない量（長期金利・人口・実質実効為替レート・エネルギー自給率）は未記入のまま残る。地層の一覧に「現在値を調べる」を置き、13件の `name`・`line`・`unit`・`where`（確認先URL）・**前回の値**を差し込んだプロンプト（`worldProbePromptText`。約3100字）を1本で回す。分野で分けないのは、月1回の作業を3回に割ると回らなくなるため——返答が途中で切れても配列に入っているものだけ入るので、足りなければもう一度回せば済む。**値・基準日・出典URL の3点が揃わなければ返せない形**にしてある：`url` が無い値は落とす（`via:'probe'` の url は生成が新しく書くので未検証。札の url と混ぜないよう via を分けてある）、`at` が無い値も落とす（**札と違い eventDate で代用しない**。基準日の無い残高は意味が違う）、推測・概算は書かせない、一覧に無い id は落とす。前回の値を差し込むのは**桁を1つ間違えた返答をこちらで気づくため**なので、「前回の値をそのまま書き写さない」を規則に入れてある。
...
**差分だけを見ない仕掛け（v11）**：出来事は「変化したもの」しか映さないので、動かないもの・溜まるものが見えなくなる。**地層**を13件だけ持ち、(1) 出来事の行には `short` のタグを1つ（行は増やさない）、(2) 今日の3件の上と朝の畳んだ行の下に**日替わりで巡回する1行**を出す（`WORLD_STRATUM_ON_COLLAPSED` で朝の分は消せる）。巡回にしてあるのは、変わらないものを毎日同じ形で見せると2週間で見えなくなるため——「今日はどれか」という小さな差を残す。値には「前回更新から N 日」を必ず出す（古いまま放置されているのが見えないと数字が嘘になる）。**「古い」の日数は地層ごと（v16 B）**：長期金利は日次、外貨準備は月次、SIPRI の軍事費と国連の人口は年次。一律90日にすると軍事費が1年中赤くなり、**赤が情報でなくなる**。`cadence` を13件に手で振り、`month` 45日／`quarter` 120日／`year` 400日で判定する（`worldStratumStale`）。値が無いものは常に古い。地層の一覧の見出しに「古い N件 / 13件」を出すので、**押す前に押す必要があるかが分かる**。
...
**プロンプトに差し込む札**は、既知の語（概念・用語・アクターの名前と別名）を辞書代わりに使って絞り、新しい順に20件まで。日本語の分かち書きを持っていないので、問いの文をそのまま部分一致にかけても当たらない。0件なら絞り込みの語を1つ入れてもらう。長さは約 3.8KB〜（札が100件に育っても20件で切るのでこのまま。通常の生成プロンプト17KBの1/4）。
**問いの画面には行動（札）を出さない**（`runs[].added` から「出来事タブで見る」で辿る）。過去の問いもこの画面の中だけ——既定に出すと「問いの在庫」ができる。札には `viaAsk` が付き、どの問いが掘り当てたかが引ける。

**まとめて貼る（v17.1）**：ソースの `stance` は22件とも空、地層の `where.url` は13件中7件が「数字の載っていないページ」（CSV・PDF・JS描画・DB形式）を指していた。**手で22回・13回入力するのは現実的でない**ので放置されていた。設定 → 世界 に貼り付け欄を1つ置き、「コピー → 貼る → 取り込む」の形で一度に入れる。
...
タブは増やさず、**出来事タブの中の型の切替**（[札][お金]）に置いた（v17 のタブ再編で出来事タブが「探す」に畳まれるとき、お金もそのまま一緒に移る。いま独立したタブを作ると作り直しになる）。**通貨ごとに区切り**（換算しないので足せない）、`from → to` の組でまとめ、**2回以上ある組を上に出す**——繰り返しがゲートの代わりで、同じ相手に何度も出している金が自然に目立つ。**合計は出さない**（「財務省 合計20兆円」を出した瞬間に、性質の違う金を足した数字が独り歩きする）。
...
**主体の盤面（v13）**：世界タブの**既定を入れ替えた**。タブは [主体][出来事][概念][予測] の4つで、既定は「主体」。この機能は「情報を集める」から作り始めて「思惑を見る」を後付けしたので、土台が札の一覧のままだった。最初に見るべきは「今日どんな出来事があったか」ではなく「**いま誰が何を動かそうとしているか**」。
...
右端の印がこの画面の核心：`述`＝述べた理由がある／`推`＝推定がある／**`・推`＝述べた理由が1件も無く推定だけ**。3つ目が一番重要（何も言わずに動いている主体）なので、琥珀色に加えて**中黒で位置もずらす**——色だけだと「述推」と並んだとき走査で拾えない。凡例は畳める（`kotoba_worldBoardLegend`）。
