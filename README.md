...
**編集画面の並び（v9.3）**：上から順に埋めれば完成する順。1 問い → 2 仮の答え → 3 タイトル → 4 相談 → 5 カード → 6 答えを壊す → 7 展開 → 8 入り。まだ使えないブロックはボタンを薄くして理由を 1 行だけ出す（`shootGate`。「カードが3枚そろうと使えます。（いま 1枚）」など）。「答えを壊す」の結果の下に仮の答えをもう一度置いてあり、上の欄と同じ内容を書き直せる（両方向に同期）。
